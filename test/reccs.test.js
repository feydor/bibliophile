...
describe('testing reccs.fetchApi', () => {
  ...
  test('fetch json from a known working url', (done) => {
    url = 'https://openlibrary.org/api/books?bibkeys=ISBN:0198239467&jscmd=data&format=json';
  ...
  test('fetch json from a broken url', (done) => {
    url = 'https://openlibrary.org/api/book?bibkeys=ISBN:0198239467&jscmd=data&format=json';
    callback = (error, result) => {
      try {
